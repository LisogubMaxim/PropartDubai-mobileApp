...
const AreaProperties = ({ onCloseModal }) => {
    const [filterPoint, setFilterPoint] = useState(areaPropertiesData.filterPoint);
        ...
        <View style={{ flex: 1 }}>
            <View style={styles.header}>
                <View style={styles.headerTitle}>
                    <TouchableOpacity onPress={onCloseModal} style={{ marginRight: 12 }}>
                        <BackWhiteButtonSvg />
                    </TouchableOpacity>
                    <Text style={styles.title}>Property in Business Bay</Text>
                </View>
                <View style={styles.search}>
                    <SearchInput placeholder="Search" />
                    <FiltersBtn callModal={() => setFiltersVisible(true)} />
                </View>
                {filterPoint && <Point text={filterPoint} onPressDelete={handleClearPoint} />}
            </View>
            <ScrollView style={styles.body} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 16 }}>
